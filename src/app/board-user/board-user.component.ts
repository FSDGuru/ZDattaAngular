...
export class BoardUserComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.passenger.passenger_seat <= 30) {
      this.passenger.amount = 6320;
    }
    ...
    this.route.paramMap.subscribe((params) => {
      this.flight_id = +params.get('flight_id')!;
    });
  ...
  saveBooking(): void {
    ...
    this.bookingService.create(data, this.usermail).subscribe({
      next: (res) => {
    ...
    this.message =
      'Booking Made successfully with PNR - ' +
      this.booking.booking_id +
      ' & ' +
      'Passenger details: ' +
      this.passenger.passenger_name +
      ' / seat:' +
      this.passenger.passenger_seat;
  ...
  newPassenger() {
    ...
    this.addedPassengers.push(this.passenger);
    this.showDataSection = true; // Show data section after adding passenger
  }

  addedPassengers: PassengerModel[] = [];
}
